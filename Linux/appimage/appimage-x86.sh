#!/bin/bash
export PUBLISHFLAGS="-r linux-x86 --no-self-contained false -p:PublishSingleFile=true -p:PublishReadyToRun=false"
make -j$(nproc) -O LTO=true STATICLINK=true SYSTEM_LIBEVDEV=false ARCH=x86

curl -SL https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-i686.AppImage -o appimagetool

mkdir -p AppDir/usr/bin
cp bin/linux-x86/Release/linux-x86/publish/Mesen AppDir/usr/bin
...
ARCH=i686 ./appimagetool AppDir/ Mesen.AppImage
